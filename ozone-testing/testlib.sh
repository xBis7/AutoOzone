...
copyTermOutToAllNodesAndClearFile() {
  ...
  if [[ $backup_dir_num == "" || $bootstrap_backup_dir_num == "" ]]
  then
...
checkRatisSnapshotIns() {
	...
	while [[ $checkpointDir_res == *"FAILED"* ]]
	do
	...
	while [[ $snap_num_on_dir_res < $snap_num_on_file_res ]]
	do
...
transferOMLeadership() {
  ...
  echo "Transferring leadership to OM: $om"
	...
	while [[ $leader_change_res == *"FAILED"* ]]
	do
...
checkKeyValuesFromFile() {
  ...
  while (( $count <= $NUM_KEYS_PER_SNAPSHOT ))
  do
  ...
  do
    ...
    then
      ...
      # because there is pagination and we can't get the whole number of results.
	    if [[ $diff_key_num != $NUM_KEYS_PER_SNAPSHOT ]]
	    then
